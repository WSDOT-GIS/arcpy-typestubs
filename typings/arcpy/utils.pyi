...
class ArgAdaptor:
    ...
    __args__ = ...
    @classmethod
    def maskargs(cls, fn): # -> _Wrapped[(...), Unknown, (*a: Unknown, **k: Unknown), Unknown]:
...
def in_index(string_index, start, end): # -> Literal[-1]:
    ...

def call_ranges(code, string_index): # -> list[tuple[Unknown, Unknown, Unknown, Unknown, Unknown | Literal[-1]]]:
    """Finds a list of what look like function calls in the currect text block
       that overlap with the current cursor position."""
    ...

def function_call_in_context(code, string_index, var_dict): # -> tuple[Unknown, Unknown | Literal[-1], list[Unknown], Unknown | Any | None] | tuple[Literal[''], Literal[-1], list[Unknown], None]:
...
CALL_STACK_MAP = ...
def split_call_string(call_string): # -> Generator[Unknown, Any, None]:
    ...

def arg_index_for_call_string(call_string): # -> tuple[int, Unknown | None, int]:
    ...

def parse_spec(call_prototype): # -> dict[Unknown, Unknown]:
    ...

def values_for_dropdown_function(args): # -> tuple[Unknown, ...]:
    ...

def get_index_and_argvals_for_dropdown_call(call_string, index): # -> tuple[Literal[-1], tuple[()], Literal[0], Literal[0]] | tuple[int, tuple[Unknown, ...], Unknown, Unknown | Literal[0]]:
    ...

def tool_popup_items(found_item, call_str, start_index): # -> Generator[dict[str, Unknown], Any, None]:
    ...

def tool_item_items(found_item, call_str, start_index): # -> Generator[dict[str, Unknown], Any, None]:
    ...

def help_tip(call_str, call_index, obj_tokens, found_item, start_index): # -> tuple[Unknown | str | LiteralString, Unknown | list[Unknown]]:
...
def py3_syntax_report(python_source, filename=...): # -> LiteralString | Literal['Python 3 OK', 'Syntax Error in Python 2']:
